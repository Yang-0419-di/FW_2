...
def index():
            ...
            if mode == "update_contract":
                ...
                c.execute("""
                    UPDATE contracts SET
                        ...
                        color_giveaway=?, bw_giveaway=?,
                        color_error_rate=?, bw_error_rate=?,
                        color_basic=?, bw_basic=?,
                        color_a3_unit_price=?, color_a3_giveaway=?,
                        color_a3_error_rate=?, color_a3_basic=?, tax_type=?
                    WHERE device_id=?
    ...
    # GET 直接帶 device_id
        ...
        if contract:
            last_color_a3, last_color, last_bw, last_time = get_last_counts(q_device, prev_year, prev_month)
